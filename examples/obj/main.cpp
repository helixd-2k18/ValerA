...
int main() {
    ...
    */
    ...
    for (size_t s = 0; s < shapes.size(); s++) { // 
        ...
        auto acceleration = std::make_shared<vlr::Acceleration>(fw, vlr::AccelerationCreateInfo{ .geometrySet = geometrySet, .initials = { primitiveCounts[s] } }); // Unknown Behaviour
    ...
    while (!glfwWindowShouldClose(manager.window)) { // 
        ...
        { // submit rendering (and wait presentation in device)
            ...
            */
            ...
            // Path Tracing...
            auto fnCommand = vkt::createCommandBuffer(fw->getDeviceDispatch(), commandPool, false, true);
